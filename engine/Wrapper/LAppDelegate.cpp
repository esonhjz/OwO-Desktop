...
namespace {
    ...
    const csmInt32 BackBufferNum = 1; // Number of back buffers
}
...
bool LAppDelegate::Initialize()
{
    ...
    // Increase window size for title bar and frame (if any)
    RECT rect;
    {
        SetRect(&rect, 0, 0, RenderTargetWidth, RenderTargetHeight);
        AdjustWindowRectEx(&rect, WS_POPUP, FALSE, WS_EX_TOPMOST | WS_EX_LAYERED);
    ...
    // Create window (Borderless: WS_POPUP, Always-on-top: WS_EX_TOPMOST, Layered/Transparency: WS_EX_LAYERED)
    ...
    // Enable Color Key transparency for black RGB(0,0,0)
    SetLayeredWindowAttributes(_windowHandle, RGB(0, 0, 0), 0, LWA_COLORKEY);
...
void LAppDelegate::StartFrame()
{
    ...
    */
    ...
    // Clear back buffer (Pure black for Color Key transparency)
...
LRESULT WINAPI LAppDelegate::MsgProc(HWND wnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONDOWN:
        if (s_instance != NULL)
        {
            ...
            // Allow dragging the window to any screen position
            ReleaseCapture();
