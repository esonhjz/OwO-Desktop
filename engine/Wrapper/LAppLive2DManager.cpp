    ...
    , _sceneIndex(0)
{
    ...
    // Auto-detect custom Live2D model (specifically Phoebe-Jubi or non-official models)
    csmInt32 targetIndex = 0;
    for (csmInt32 i = 0; i < _modelDir.GetSize(); ++i)
    {
        const csmString& name = _modelDir[i];
        if (name == "Phoebe-Jubi")
        {
            targetIndex = i;
            break;
        }
        else if (strcmp(name.GetRawString(), "Haru") != 0 &&
                 strcmp(name.GetRawString(), "Hiyori") != 0 &&
                 strcmp(name.GetRawString(), "Mao") != 0 &&
                 strcmp(name.GetRawString(), "Mark") != 0 &&
                 strcmp(name.GetRawString(), "Natori") != 0 &&
                 strcmp(name.GetRawString(), "Ren") != 0 &&
                 strcmp(name.GetRawString(), "Rice") != 0 &&
                 strcmp(name.GetRawString(), "Wanko") != 0)
        {
            targetIndex = i;
        }
    }
    _sceneIndex = targetIndex;
